...
public class Citoyen implements Serializable {
    ...
    public Citoyen(int id, String no, String p, Naissance n, String s, Mairie m){
    	identifiant = id;
    	nom = no;
    	nomUsage = no;
    	prenom = p;
    ...
    public boolean ageLegal(int age) {			//vérification de l'âge de la personne en fonction d'une limitation appliquée
    	if(calculAge()>=age) {					//calcul de l'âge du citoyen, si supérieur ou égal à la limitation
    		return true;						//on retourne vrai
    	}
    	return false;							//sinon on retourne faux, le citoyen est plus jeune que l'âge légal pour effectuer une action
    }
    ...
    public void affichage() {					//affichage des données et informations d'un citoyen
    	...
    	if (estMarie()==true) {					//si la personne est mariée, on indique les informations qui suivent
    		...
    		System.out.println("Date du mariage : "+mariage.get(i).dateMariage);
